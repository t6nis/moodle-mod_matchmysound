...
$url = new moodle_url('/mod/matchmysound/grade.php', array('id' => $cm->id));
if ($mode !== 'all') {
    $url->param('mode', $mode);
}
$PAGE->set_url($url);
...
$module = array(
    'name'      => 'mod_matchmysound_submissions',
    'fullpath'  => '/mod/matchmysound/submissions.js',
    'requires'  => array('base', 'yui2-datatable'),
    'strings'   => array(),
);

$PAGE->requires->js_init_call('M.mod_matchmysound.submissions.init', array(), true, $module);

$submissionquery = '
    SELECT s.id, u.firstname, u.lastname, u.id AS userid, s.datesubmitted, s.gradepercent
    FROM {matchmysound_submission} s
    INNER JOIN {user} u ON s.userid = u.id
    WHERE s.matchmysoundid = :matchmysoundid
    ORDER BY s.datesubmitted DESC
';

$submissions = $DB->get_records_sql($submissionquery, array('matchmysoundid' => $matchmysound->id));
...
<noscript>
    ...
    <style type="text/css">
        #matchmysound_submissions_table_container { display: block !important; }
    </style>
</noscript>

<div id="matchmysound_submissions_table_container" style="display:none">
    <table id="matchmysound_submissions_table">
        <thead>
            <tr>
                <th>User</th>
                <th>Date</th>
